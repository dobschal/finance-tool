...
export function findCategoryForEntry (entry: Entry): Optional<Category> {
  return categories.value.find(category => {
    ...
    function includesOneOf (...keys: Array<string>): boolean {
    ...
    try {
      ...
      if (category.filterOptions.includesOneOf != null && (category.filterOptions?.includesOneOf?.length ?? 0) > 0) {
        entryMatches = entryMatches || includesOneOf(...category.filterOptions.includesOneOf)
      }
      if (category.filterOptions.includesAllOf != null && (category.filterOptions?.includesAllOf?.length ?? 0) > 0) {
...
export function getCategories (): Array<CategoryDto> {
  ...
  const entriesWithoutCategory = entries.filter(entry => entry.category == null)
